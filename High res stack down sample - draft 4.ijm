...
//There are 6 options, so make label and default array of 6
n = 6;
labels = newArray(n);
defaults = newArray(n);
...
defaults[4] = true;
labels[5] = "Add steps to output file name";
defaults[5] = true;
...
if (Dimensions == 1){
	voxelDim = getNumber("By what factor would you like to downsample voxel simensions?", 2); 
}
...
for (i=0; i<fileList.length; i++) {
	...
	for(a=1; a<nStacks+1; a++) {	
		...
		run("Bio-Formats Importer", "open=file color_mode=Default view=[Standard ImageJ] stack_order=Default series_"+d2s(a,0)); 
